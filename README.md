...
- **单一目标转发**：客户端请求的路径与查询参数原样保留，转发到配置的目标地址
- **请求头处理**：
...
├── proxy.config.json      # 代理配置（目标地址 / 端口 / 附加请求头）
...
| 配置项 | 类型 | 说明 |
| --- | --- | --- |
| `target` | string | 转发目标地址（必填），支持 http/https，如 `https://api.example.com` |
